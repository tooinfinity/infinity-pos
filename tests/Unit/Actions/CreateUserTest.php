...
it('rejects creating a user without users create permission', function (): void {
    resolve(CreateUser::class)->handle(User::factory()->create(), CreateUserData::from([
        'name' => 'Test User',
        'email' => 'example@email.com',
        'password' => 'password',
    ]));
...
it('rejects assigning roles without users assign roles permission', function (): void {
    $actor = User::factory()->create();
    $creatorRole = Role::findOrCreate('User creator', 'web');
    $creatorRole->givePermissionTo(PermissionModel::findOrCreate(Permission::UsersCreate->value, 'web'));

    $actor->assignRole($creatorRole);
    Role::findOrCreate(RoleName::Cashier->value, 'web');
